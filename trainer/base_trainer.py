...
__description__ = 'Contains the base class that prepares the Pytorch Lightning trainer.'

from typing import Union, Literal, Optional, Callable, Tuple
...
class BaseTrainer:
    ...
    def __init__(self,
        ...
        if train_device == 'gpu':
            try:
                ...
                logger.warning('Tried to utilize Tensor Cores, but none found.')
    ...
    def _build_PL_trainer(self,
                          state: int, 
                          save_state: Callable[[None], Tuple[str, type]]) -> type:
