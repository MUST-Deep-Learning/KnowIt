...
[2] "Temporal Convolutional Networks and Forecasting" by Francesco Lässig, 2021
...
__description__ = 'Example of a Temporal Convolutional Network (TCN).'

import torch.nn as nn
import numpy as np
from torch.nn.utils import weight_norm

from knowit.helpers.logger import get_logger
logger = get_logger()
...
class Model(nn.Module):
    ...
    depth : int, default=-1
        The desired number of convolutional blocks to include in the TCN stage.
    ...
    depth : int
        The number of convolutional blocks in the TCN stage.
    num_filters : int
    ...
    def _build_fcn(self, dilation_base=2):
        """Builds the fully convolutional stage of the TCN. """
        depth = self.depth
        if depth == -1:
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, n_inputs, n_outputs, kernel_size,
        ...
        if normalization == 'weight':
            ...
            layers.append(ClipRightPad(padding))
...
class ClipRightPad(nn.Module):
    ...
    ----------
    clip_size : int
        The number of time steps or elements to clip from the right end of the input tensor.
    ...
    def forward(self, x):
        ...
        -------
        torch.Tensor
            Output tensor with right-side padding removed, of shape (batch_size, channels, sequence_length - clip_size).
        """
